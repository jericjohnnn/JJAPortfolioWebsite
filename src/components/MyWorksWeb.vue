<template>
  <div class="w-full">
    <VueFlicking :options="{
      ...
      autoInit: true,
      bounce: '100%',
    }" :plugins="plugins" class="py-4 w-full">
      <div v-for="(item, index) in carouselItems" :key="index" class="w-full sm:w-5/6 md:w-1/2 lg:w-1/3">
        <Card class="w-full h-full overflow-hidden">
          ...
          <template #content>
            ...
            <div class="flex flex-wrap gap-2 ">
              <span v-for="technology in item.technologies" :key="technology">
                <img :src="technology" alt="Project technologies" class="h-8 w-full" />
              </span>
            </div>
          </template>
          <template #footer>
            <div class="flex flex-wrap gap-2">
              <a v-for="(link, linkIndex) in item.links" :key="linkIndex" :href="link.url" target="_blank"
                ...
                <box-icon :type="link.icon === 'globe' ? '' : 'logo'" :name="link.icon" color='#F6E332'
                  animation='tada'></box-icon>
              </a>
            </div>
          </template>
        </Card>
...
const carouselItems = [
  {
    image: '/works_dev/tudlo.webp',
    title: 'Tudlo',
    description: 'An online hub where students can easily find, book, and connect with the right tutors for their needs.',
    technologies: [
